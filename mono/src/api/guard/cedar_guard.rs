...
pub async fn cedar_guard(
    ...
    let user = req
        ...
        .get::<LoginUser>()
        .cloned()
        .ok_or_else(|| MegaError::with_message("Missing LoginUser injection"))?;
...
async fn authorize(
    ...
    let principal =
        saturn::util::EntityUid::from(EntityUid::from_type_name_and_id(role_entity, user_entity));
    let action = saturn::util::EntityUid::from(EntityUid::from_type_name_and_id(
        actiontype_entity,
        action_entity,
    ));
    let resource = saturn::util::EntityUid::from(EntityUid::from_type_name_and_id(
