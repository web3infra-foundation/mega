...
async fn build_tree_from_map(
    ...
    // Create a simple root tree object
    // This is a simplified implementation, should recursively create subtrees in practice
    let mut tree_items = Vec::new();
    for path_buf in temp_index.tracked_files() {
    ...
    let tree = Tree::from_tree_items(tree_items).map_err(|e| e.to_string())?;
...
fn rebuild_index_from_tree(tree: &Tree, index: &mut Index, prefix: &str) -> Result<(), String> {
    for item in &tree.tree_items {
        ...
        if let TreeItemMode::Tree = item.mode {
            ...
            rebuild_index_from_tree(&subtree, index, full_path.to_str().unwrap())?;
        } else {
