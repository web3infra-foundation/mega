...
pub async fn fetch_repository(remote_config: RemoteConfig, branch: Option<String>) {
    ...
    let transaction_result = db.transaction(|txn| Box::pin(async move {
        ...
        for r in &refs {
            ...
            let old_oid = Branch::find_branch_with_conn(txn, &full_ref_name, None).await
        ...
        if let Some(remote_head) = remote_head {
            if let Some(remote_head_ref) = ref_heads.iter().find(|r| r._hash == remote_head._hash) {
                if let Some(remote_head_branch) = remote_head_ref._ref.strip_prefix("refs/heads/") {
