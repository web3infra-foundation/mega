...
use orion_server::scheduler::{LogReadError, create_log_file, read_log_segment_raw};
use std::io::Write;
use std::sync::Mutex;
...
async fn test_log_segment_read_basic() {
    ...
    let seg = read_log_segment_raw("segment-basic", 0, 5)
        .await
        .expect("segment");
    assert_eq!(seg.data, "Hello");
    let seg2 = read_log_segment_raw("segment-basic", seg.next_offset, 1024)
        .await
        .expect("segment2");
...
async fn test_log_segment_zero_len_metadata() {
    ...
    let seg = read_log_segment_raw("segment-zero", 0, 0)
        .await
        .expect("segment");
    assert_eq!(seg.len, 0);
