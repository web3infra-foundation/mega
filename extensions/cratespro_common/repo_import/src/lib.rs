...
impl ImportContext {
    ...
    pub async fn import_from_env_vars(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        if output.status.success() {
            ...
            tracing::info!("out import_from_env_vars");
            Ok(())
        } else {
            // 处理错误情况
