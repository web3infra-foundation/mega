...
impl Display for GitTypeCounter{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "commit:{}  tree:{}   blob:{}  tag:{}\tref_delta :{}\tofs_dalta:{}",self.commit, self.tree, self.blob ,self.tag, self.ref_delta, self.ofs_delta)?;

        writeln!(f,"base_object:{} \t delta_object:{} \t",self.base_count(), self.delta_count())
    }
}
...
impl Display for DecodeCounter{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f,"base:{} delta:{}, cache_hit: :{},Storage operation:{},delta_depth:{}",self.base,self.delta,self.cache_hit,self.db_look,self.delta_depth)
    }
}
