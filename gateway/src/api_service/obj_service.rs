...
pub async fn get_blob_objects(
    ...
    let blob_data = match storage.get_node_by_hash(object_id).await {
        Ok(Some(node)) => {
...
pub async fn get_tree_objects(
    ...
    let tree_id = if let Some(object_id) = object_id {
        ...
        let commit_id = match storage.search_refs(repo_path).await {
         ...
         match storage.get_commit_by_hash(&commit_id).await {
            Ok(Some(commit)) => commit.tree,
            _ => return Err((StatusCode::NOT_FOUND, "Tree not found".to_string())),
        }
    };

    let tree_data = match storage.get_node_by_hash(&tree_id).await {
        Ok(Some(node)) => {
...
pub async fn get_objects_data(
    ...
    let node = match storage.get_node_by_hash(object_id).await {
        Ok(Some(node)) => node,
        _ => return Err((StatusCode::NOT_FOUND, "Blob not found".to_string())),
    };
