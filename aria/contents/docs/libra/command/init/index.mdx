...
-   `--shared[=(false|true|umask|group|all|world|everybody|<perm>)]`  
    Set repository permissions for multi-user access:  
    - `false` / `umask`: use default `umask` of the system.  
    - `true` / `group`: group-writable.  
    - `all` / `world` / `everybody`: writable by all.  
    - `<perm>`: 4-digit octal starts with `0`.  