...
pub const ADMIN_CACHE_KEY: &str = "mega:admin:list";
...
impl MonoApiService {
    ...
    pub async fn invalidate_admin_cache(&self) {
        ...
        if let Err(e) = conn.del::<_, ()>(ADMIN_CACHE_KEY).await {
    ...
    async fn get_admins_from_cache(&self) -> Result<Vec<String>, MegaError> {
        let mut conn = self.git_object_cache.connection.clone();
        let data: Option<String> = conn.get(ADMIN_CACHE_KEY).await?;
    ...
    async fn cache_admins(&self, admins: &[String]) -> Result<(), MegaError> {
        ...
        conn.set_ex::<_, _, ()>(ADMIN_CACHE_KEY, json, ADMIN_CACHE_TTL)
            .await?;
        Ok(())
    }
}
