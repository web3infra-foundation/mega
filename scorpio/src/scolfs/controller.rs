...
use libra::utils::path_ext::PathExt;
use libra::utils::{path, util};
...
use super::lfs::lfs_client::LFSClient;
use super::utils;
use super::lfs_util;
...
pub async fn execute(cmd: LfsCmds) {
    ...
    let attr_path = attr_path.to_str().unwrap();
    match cmd {
        LfsCmds::Track { pattern } => { // TODO: deduplicate
            match pattern {
                ...
                None => {
                    let lfs_patterns = lfs_util::extract_lfs_patterns(attr_path).unwrap();
        ...
        LfsCmds::LsFiles { long, size, name_only} => {
            ...
            for entry in entries {
                ...
                if lfs_util::is_lfs_tracked(&path_abs) {
                    let data = storage.get(&entry.hash).unwrap();
                    if let Some((oid, lfs_size)) = lfs_util::parse_pointer_data(&data) {
                        let is_pointer = lfs_util::parse_pointer_file(&path_abs).is_ok();
...
/// Get the current ref branch name.
...
fn convert_patterns_to_workdir(patterns: Vec<String>) -> Vec<String> {
    ...
    patterns.into_iter().map(|p| {
        let mut pattern = mount_path.clone();
        pattern.push(p);
        pattern.to_string_or_panic()
...
fn add_lfs_patterns(file_path: &str, patterns: Vec<String>) -> io::Result<()> {
    ...
    let lfs_patterns = lfs_util::extract_lfs_patterns(file_path)?;
