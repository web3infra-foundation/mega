  ...
  var caAgent = new http.Agent('localhost:9999')
   caAgent.request('GET', '/api/certificates/ca').then(
    function (res) {
      ...
      // myKey = new crypto.PrivateKey({ type: 'rsa', bits: 2048 })
      // var pkey = new crypto.PublicKey(myKey)
      return caAgent.request('POST', `/api/certificates/${username}`, null, "")
    }
  ).then(
    function (res) {
      if (res.head.status !== 200) {
      ...
      myKey = new crypto.PrivateKey(res.body.toString())
      println('===============')
      println('Hub Key')
      println('===============')
      println(res.body.toString())
      // myKey = res.body.toString();
      // myKey = new crypto.PrivateKey({ type: 'rsa', bits: 2048 })
      return caAgent.request('GET', `/api/certificates/${username}`)
    }
  ).then(
    function (res) {
      if (res.head.status !== 200) {
