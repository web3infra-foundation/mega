...
impl RelayMessage {
    ...
    pub fn new_event_box(subscription_id: SubscriptionId, event: Box<NostrEvent>) -> Self {
        Self::Event {
            subscription_id,
            event,
        }
    ...
    pub fn from_value(msg: Value) -> Result<Self, MessageHandleError> {
        ...
        if v[0] == "EVENT" {
            ...
            return Ok(Self::Event {
                ...
                event: Box::from(NostrEvent::from_value(v[2].clone())?),
            });
        }
        ...
        if v[0] == "EOSE" {
            ...
            return Ok(Self::EndOfStoredEvents(SubscriptionId::new(subscription_id)));
        }
...
mod tests {
    ...
    fn test_handle_valid_event() {
        ...
        let handled_event = NostrEvent::new_with_timestamp(key_pair, 1111, NostrKind::Mega, tags, content);
