...
use venus::{hash::SHA1, internal::{object::{types::ObjectType, utils}, pack::{entry::Entry, header::EntryHeader}}};
...
impl StorageProvider for GitStorage {
    ...
    async fn get_entry_by_sha1(&self, sha1_vec: Vec<&str>) -> Result<Vec<Entry>, MegaError> {
        ...
        for sha1 in sha1_vec {
            let data = self.rawobj_storage.get(sha1).await.unwrap();
...
impl GitStorage {
    pub async fn new() -> Self {
        GitStorage {
            rawobj_storage: raw_storage::init("git-objects".to_owned()).await,
        }
    }
}
