    ...
    <TreeItemProvider {...getContextProviderProps()}>
      <TreeItemRoot {...getRootProps(other)}>
        <TreeItemContent {...getContentProps()}>
          ...
          {/* icon */}
          <React.Fragment>
            <IconButton onClick={handleClick} aria-label='collapse item' size='small'></IconButton>
          </React.Fragment>
      ...
      const loadChildren = async () => {
        try {
          ...
          if (node.path && node.path !== '' && node.path !== undefined) {
            responseData = await fetch(`/api/tree?path=${node.path}`).then((response) => response.json())
          } else {
            responseData = await fetch(`/api/tree?path=${reqPath}`).then((response) => response.json())
          }
