...
use fuser::{
    FileType, Filesystem, KernelConfig, ReplyAttr, ReplyCreate, ReplyData,
...
impl RLFileSystem {
    pub fn new(remote_url: &str, fs_name: &str, direct_io: bool, remote_root: &str) -> Self {
        let fs_name = fs_name.to_owned();
        let rt=Arc::new(tokio::runtime::Builder::new_multi_thread().enable_all().build().unwrap());
        Self {
            fs_name: fs_name.clone(),
            rt:rt.clone(),
            tmp_manager: TemporaryFileManager::new(fs_name),
            inodes: HashMap::new(),
...
impl Filesystem for RLFileSystem {
    ...
    fn getattr(&mut self, _req: &Request<'_>, ino: u64, reply: ReplyAttr) {
        match self.inodes.get(&ino) {
            Some(inode) => reply.attr(&Duration::new(0, 0), &inode.file_attr()),
            None => reply.error(libc::ENOENT),
        }
    }
    ...
    fn write(
        ...
        self.tmp_manager.append_content(ino, data,inode.attr.id.clone());
        inode.attr.mtime = SystemTime::now();
    ...
    fn fallocate(
        ...
        match self.inodes.get_mut(&ino) {
            Some(inode) => {
                self.tmp_manager.fallocate(ino, mode, offset, length,inode.attr.id.clone());
