...
mod test {
    ...
    // static REDIS_CONTAINER: Lazy<OnceCell<ContainerAsync<GenericImage>>> = Lazy::new(OnceCell::new);

    // async fn kafka_container() -> &'static ContainerAsync<GenericImage> {
    //     REDIS_CONTAINER
    //         .get_or_init(|| async {
    ...
    //     let container = kafka_container().await;
    ...
    pub async fn test_loop_consume() {
        ...
        // let broker = kafka_bootstrap_servers().await;
    ...
    pub async fn test_station_process() {
        ...
        // let crate_repo_topic ="test-topic-crate_repo".to_owned();
        init_tracing();
        ...
        let task2 = tokio::spawn(async move {
            let crate_repo_telescope =
                Telescope::new(BROKER, CONSUMER_GROUP, "test-topic-crate_repo");
