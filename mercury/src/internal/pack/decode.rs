...
impl Pack {
    ...
    pub fn decode(
        &mut self,
        pack: &mut (impl Read + BufRead + Seek + Send),
        mem_size: usize,
        tmp_path: PathBuf,
    ) -> Result<(), GitError> {
        ...
        while i <= self.number {
            let r: Result<CacheObject, GitError> =
                self.decode_pack_object(&mut render, &mut offset);
            match r {
    ...
    fn rebuild_delta(mut delta_obj: CacheObject, base_obj: Arc<CacheObject>) -> CacheObject {
        ...
        delta_obj.hash =
            utils::calculate_object_hash(delta_obj.obj_type, &delta_obj.data_decompress);
